...
// Using [\s\S]*? to handle newlines between \" and ON
sql = sql.replace(/CREATE POLICY \"([^\"]+)\"\s+ON\s+([\w\.]+)/gim, 'DROP POLICY IF EXISTS \"$1\" ON $2;\nCREATE POLICY \"$1\" ON $2');
...
fs.writeFileSync('SALVACAO_DO_BANCO_SUPREMA.sql', sql);
console.log('SALVACAO_DO_BANCO_SUPREMA.sql criado com sucesso!');
