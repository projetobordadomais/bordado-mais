...
CREATE TABLE IF NOT EXISTS public.strategy_conversations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE,
  title TEXT,
...
CREATE INDEX IF NOT EXISTS idx_profiles_premium_expires ON public.profiles(premium_expires_at);
CREATE INDEX IF NOT EXISTS idx_profiles_free_cycle_expires ON public.profiles(free_cycle_expires_at);
...
CREATE INDEX IF NOT EXISTS idx_generations_expires_at ON public.generations(image_expires_at);
...
CREATE INDEX IF NOT EXISTS idx_financial_records_date ON public.financial_records(record_date);
CREATE INDEX IF NOT EXISTS idx_financial_records_type ON public.financial_records(type);

-- pricing_calculations
CREATE INDEX IF NOT EXISTS idx_pricing_user_id ON public.pricing_calculations(user_id);

-- strategy_conversations
CREATE INDEX IF NOT EXISTS idx_strategy_user_id ON public.strategy_conversations(user_id);
...
DROP TRIGGER IF EXISTS update_strategy_conversations_updated_at ON public.strategy_conversations;
CREATE TRIGGER update_strategy_conversations_updated_at
  BEFORE UPDATE ON public.strategy_conversations
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

DROP TRIGGER IF EXISTS update_plan_config_updated_at ON public.plan_config;
...
DROP POLICY IF EXISTS "Authenticated can view active packages" ON credit_packages;
...
DROP POLICY IF EXISTS "Admin can manage packages" ON credit_packages;
...
DROP POLICY IF EXISTS "Users can manage own chat sessions" ON strategy_conversations;
...
DROP POLICY IF EXISTS "Users can view own coupon usages" ON public.user_coupons;
CREATE POLICY "Users can view own coupon usages" ON public.user_coupons FOR SELECT 
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can view all coupon usages" ON public.user_coupons;
CREATE POLICY "Admins can view all coupon usages" ON public.user_coupons FOR SELECT 
...
CREATE POLICY "Partners can view own commissions" ON public.partner_commissions FOR SELECT
    USING (auth.uid() = partner_id);

DROP POLICY IF EXISTS "Admins can fully manage partner commissions" ON public.partner_commissions;
CREATE POLICY "Admins can fully manage partner commissions" ON public.partner_commissions
    USING (public.is_admin());
...
DROP POLICY IF EXISTS "Anyone can view campaign config" ON public.campaign_config;
CREATE POLICY "Anyone can view campaign config" ON public.campaign_config FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Admins can manage campaign config" ON public.campaign_config;
...
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS aceitou_termos BOOLEAN DEFAULT false;
